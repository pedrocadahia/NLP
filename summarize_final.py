...
def summarize(es_search_host, search_index, doc_name, criterion_name):
    ...
    text = map_many(text, par(del_page_num, RegEx=r"([A-Z]{1,2}[/][\d])\w"),
                    par(del_page_num, RegEx=r"\b(\d{1,2}/\d{1,2})\b"))
    txt = ''.join(text)

    nodes = sentences(txt, lang)  # lang extraer de elastic
    # nodes = [word for word in nodes if len(word) >100]
    edges = connect(nodes, lang)  # lang extraer elastic
    ...
    vec_list = (phrases, get_phrase(nodes, phrases, text), scores, scroll)
    #  filter results by length
    # index = np.where(np.array(list(map(len,phrases)))==length)# params: length
    # [x[index] for x in vec_list]
